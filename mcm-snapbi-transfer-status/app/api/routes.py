...
from app.api.snapbi_client import call_snapbi_transfer_status
...
@router.post("/SnapBITransferStatus", summary="SNAPBI Get Transfer Status")
async def snapbi_transfer_status(
    request: Request,
    ...
    endpoint_transfer_status_result = await db.execute(select(ApiEndpoint).where(ApiEndpoint.name == 'SnapBI Transfer Status'))
    endpoint_transfer_status = endpoint_transfer_status_result.scalar()
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.request(
                ...
                url=f"{transaction_signature_service}?HTTPMethod=POST&EndpointUrl={endpoint_transfer_status.path}",
            ...
            snap_bi_response_data = await call_snapbi_transfer_status(db, access_token, request_body, signature_data, timestamp_data, X_PARTNER_ID, CHANNEL_ID, endpoint_transfer_status)
