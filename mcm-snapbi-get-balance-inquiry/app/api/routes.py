...
from app.api.snapbi_client import call_snapbi_balance_inquiry
from app.services.auth import verify_token
from app.models.models import ApiEndpoint, ApiCredential, ApiToken
from app.core.config import DEBUG_MODE, X_PARTNER_ID, CHANNEL_ID
...
@router.post("/SnapBIGetBalanceInquiry", summary="Get SNAPBI BalanceInquiry")
async def get_snapbi_balance_inquiry(
    ...
    endpoint_balance_inquiry_result = await db.execute(select(ApiEndpoint).where(ApiEndpoint.name == 'SnapBI Get Balance Inquiry'))
    endpoint_balance_inquiry = endpoint_balance_inquiry_result.scalar()
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.request(
                ...
                url=f"{transaction_signature_service}?HTTPMethod=POST&EndpointUrl={endpoint_balance_inquiry.path}",
            ...
            snap_bi_response_data = call_snapbi_balance_inquiry(access_token, request_body, signature_data, timestamp_data, X_PARTNER_ID, CHANNEL_ID, endpoint_balance_inquiry)
