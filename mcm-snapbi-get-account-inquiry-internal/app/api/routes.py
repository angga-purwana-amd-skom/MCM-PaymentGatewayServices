...
from app.api.snapbi_client import call_snapbi_balance_inquiry_internal
...
@router.post("/SnapBIGetBalanceInquiryInternal", summary="Get SNAPBI BalanceInquiry Internal")
async def get_snapbi_balance_inquiry_int(
    request: Request,
    ...
    endpoint_balance_inquiry_internal_result = await db.execute(select(ApiEndpoint).where(ApiEndpoint.name == 'SnapBI Get Balance Inquiry Internal'))
    endpoint_balance_inquiry_internal = endpoint_balance_inquiry_internal_result.scalar()

    if not endpoint_balance_inquiry_internal:
        raise ValueError("SNAPBI Get Balance Inquiry Endpoint Internal not found in database")    

    try:
        async with httpx.AsyncClient() as client:
            response = await client.request(
                method=request.method,
                url=f"{transaction_signature_service}?HTTPMethod=POST&EndpointUrl={endpoint_balance_inquiry_internal.path}",
            ...
            snap_bi_response_data = await call_snapbi_balance_inquiry_internal(db, access_token, request_body, signature_data, timestamp_data, X_PARTNER_ID, CHANNEL_ID, endpoint_balance_inquiry_internal)
