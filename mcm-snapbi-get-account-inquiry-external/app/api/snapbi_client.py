...
async def call_snapbi_balance_inquiry_external(db: AsyncSession, access_token: str, request_body: dict, signature_data: str, timestamp_data: str, x_partner_id: str, channel_id: str,  endpoint_balance_inquiry_external: ApiEndpoint) -> str:
    ...
    if not endpoint_balance_inquiry_external:
        raise ValueError("SNAPBI Get Balance Inquiry External Endpoint not found in database")
    ...
    ep_port = "" if endpoint_balance_inquiry_external.port == "" else f":{endpoint_balance_inquiry_external.port}"
    url = f"https://{endpoint_balance_inquiry_external.host}{ep_port}{endpoint_balance_inquiry_external.path}"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=payload, ssl=not DEBUG_MODE) as response:
            ...
            event_log = ApiEventLog(
                endpoint_id=endpoint_balance_inquiry_external.id,
                request_method="POST",
