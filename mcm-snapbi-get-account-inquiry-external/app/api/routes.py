...
from app.api.snapbi_client import call_snapbi_balance_inquiry_external
...
async def get_snapbi_balance_inquiry_ext(
    ...
    endpoint_balance_inquiry_external_result = await db.execute(select(ApiEndpoint).where(ApiEndpoint.name == 'SnapBI Get Balance Inquiry External'))
    endpoint_balance_inquiry_external = endpoint_balance_inquiry_external_result.scalar()

    if not endpoint_balance_inquiry_external:
        raise ValueError("SNAPBI Get Balance Inquiry Endpoint External not found in database")    

    try:
        async with httpx.AsyncClient() as client:
            response = await client.request(
                method=request.method,
                url=f"{transaction_signature_service}?HTTPMethod=POST&EndpointUrl={endpoint_balance_inquiry_external.path}",
            ...
            snap_bi_response_data = await call_snapbi_balance_inquiry_external(db, access_token, request_body, signature_data, timestamp_data, X_PARTNER_ID, CHANNEL_ID, endpoint_balance_inquiry_external)
